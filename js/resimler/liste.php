...
if($_POST){
    ...
    $klasor = $_POST["klasor"];
    $klasorekle2 = $sunucubaglan->prepare("INSERT INTO resimklasor SET klasor = ?");
    $klasorekle = $klasorekle2->execute(array($klasor));
}
if (isset($_GET['sil']))
	{
		
		$urun_silme = $sunucubaglan->prepare("DELETE FROM resimklasor WHERE id = :id");
...
<div class="container">
  <div class="row">
      ...
      <?php
$query= $sunucubaglan -> prepare("SELECT * FROM resimklasor order by id desc"); // resimler tablaosunda bulunan atadigimiz id ait bilgi varsa resimleri gostersin.
$query-> execute();
...
<?php
$query= $sunucubaglan -> prepare("SELECT * FROM resimlertoplu  ORDER BY id DESC limit 20"); // resimler tablaosunda bulunan atadigimiz id ait bilgi varsa resimleri gostersin.
$query-> execute();
$musteriler = $query -> fetchAll(); ?>
