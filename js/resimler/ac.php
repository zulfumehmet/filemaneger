...
header("location:http://www.alpersahinoglu.av.tr/"); //Sql injection girişimi yakalandığında yönlendiriyoruz.
exit;
}
...
header("location:http://www.alpersahinoglu.av.tr/");
exit;	
}
...
<?php
$query= $sunucubaglan -> prepare("SELECT * FROM resimlertoplu where ilanid='$gelen_kod' ORDER BY id DESC"); // resimler tablaosunda bulunan atadigimiz id ait bilgi varsa resimleri gostersin.
$query-> execute();
  ...
  <div class="card-body">
     <a href="sil.php?id=<?=$dizi["id"]?>" onclick="return confirm('Silmek istediğinize emin misiniz ?')"><button type="button " class="btn btn-primary btn-sm">Sil</button></a>
				  <button type="button" class="btn btn-primary btn-sm" onclick="copyToClipboard('#p<?=$dizi["id"]?>')">Kopyala</button>
				   <p id="p<?=$dizi["id"]?>"  class="d-none d-print-block"><?=$dizinyolu;?><?php echo $gelen_kod ?>/<?=$dizi["resim_adi"]?></p>
