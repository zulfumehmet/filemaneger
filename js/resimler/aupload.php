  ...
  if ($handle->uploaded) {
    ...
    if ($handle->processed) {
      ...
      $this_upload['large'] = $handle->file_dst_name;
        ...
        //Limitlendirme
        $limit = $sunucubaglan->query("SELECT * FROM resimlertoplu where ilanid='$gelen_kod'");
        $limit = $limit->rowCount(); 
        if ($limit<100) // burasi ile 10 fotograftan fazla resim yuklemesine izin vermeyelim limitide resim ilan id si ile yapiyoruz. Cunku resim ilan id aynı olanlari sinirlayabiliriz. resim id si her fotograf icin bir tanedir.
        {
            $buyuk = $this_upload["large"];
            $kucuk = $this_upload["small"];
            
        // Burasi da her bir resim icin calisin.
        $sorgu = $sunucubaglan->prepare("INSERT INTO resimlertoplu(ilanid, resim_adi, tarih, kucuk) VALUES(?, ?, ?, ?)");
        $sorgu->bindParam(1, $gelen_kod, PDO::PARAM_STR);
        $sorgu->bindParam(2, $buyuk, PDO::PARAM_STR);
        $sorgu->bindParam(3, $tarihsaat, PDO::PARAM_STR);
        $sorgu->bindParam(4, $kucuk, PDO::PARAM_STR);
